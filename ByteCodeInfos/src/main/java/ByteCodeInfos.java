...
public class ByteCodeInfos {
    ...
    private static void extractMethod(String key,SootMethod sootMethod,String className) {
        ...
        if (annotationInterfaceContent!=null ) {
            ...
            synchronized (ByteCodeInfos.monitorWrite) {
                    line=splittedPath[splittedPath.length-3]+"/"+splittedPath[splittedPath.length-2]+"!"+ 
                            annotationInterfaceContent.getName().replace(".java", "")+"!"+
                            getClassName(className, annotationInterfaceContent.getName())+ "!" +sootMethod.getDeclaration() +"!"+ annotationInterfaceContent.getStartLine()+"!"+annotationInterfaceContent.getEndLine();
                ...
                for (Enumeration<JarEntry> entries = jar.entries(); entries.hasMoreElements(); ) {
                    ...
                    if (entry.getName().endsWith(".class") ) {
                        if (entry.getName().contains("/")) {
                            classList.add(entry.getName().replace(".class", "").replace("/", "."));
                        }
                        else
    ...
    private static String getClassName(String className, String javaFileName){
        return className.replace("_"+javaFileName.replace(".java", "")+".", "");
    }
}
